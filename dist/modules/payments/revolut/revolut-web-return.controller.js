...
exports.webPaymentReturn = webPaymentReturn;
...
|
*/
async function webPaymentReturn(req, res) {
    try {
        /*
        |--------------------------------------------------------------------------
        | Purchase Reference
        ...
        */
        const frontendUrl = (process.env
            ...
            "").replace(/\/+$/, "");
        if (!frontendUrl) {
            console.error("WEB PAYMENT RETURN ERROR: attendee web URL is not configured.");
            return res.status(500).send("Attendee web application URL is not configured.");
        }
        const dashboardUrl = `${frontendUrl}/attendee/dashboard?purchase=${encodeURIComponent(purchase.id)}`;
        return res.redirect(302, dashboardUrl);
    }
    catch (error) {
        console.error("WEB PAYMENT RETURN ERROR:", error);

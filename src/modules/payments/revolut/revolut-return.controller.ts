...
export async function paymentReturn(
  ...
  try {
    ...
    */
    ...
    const deepLink =
      `wowyou://payment-return?purchase=${encodeURIComponent(
        purchase.id,
      )}`;
...
export async function subscriptionReturn(
  req: Request,
  res: Response,
) {
  try {
    ...
    */
    ...
    if (
      ...
      return res.status(400).send(
        "Invalid subscription return platform.",
      );
    }
    ...
    console.error(
      "SUBSCRIPTION RETURN ERROR:",
      error,
    );

    return res.status(500).send(
      "Unable to process subscription return.",
    );
  }
}